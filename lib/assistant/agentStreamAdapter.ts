...
export type PlaceCardsData = {
  ...
  items: PlaceCardItem[];
  // Short natural-language plan summary generated by the agent from the tool
  // output. Present on itinerary/recommendation turns; absent otherwise.
  summary?: string;
  // Target date(s) of the plan, taken from the itinerary anchor time(s), as
  // ISO dates (YYYY-MM-DD). A single day sets both to the same value; a
  // multi-day plan spans the earliest and latest day.
  startDate?: string;
  endDate?: string;
};
...
function parsePlaceCardsData(value: unknown): PlaceCardsData | null {
  ...
  const result: PlaceCardsData = { source: value.source, items };
  ...
  const startDate = asString(value.start_date ?? value.startDate).trim();
  const endDate = asString(value.end_date ?? value.endDate).trim();
